---
wb-type: faction
wb-id: exiled-blades
name: Exiled Blades
aliases:
- Exiled Blades of the Crimson Wastes
type: militant
founded: null
dissolved: null
headquarters: '[[Crimson Wastes]]'
leader: null
territory:
- '[[Crimson Wastes]]'
influence: mercenary
goals:
- Achieve harmony by surgically excising all dissonance with steel
- Prove martial supremacy through duels of honor
- Offer disciplined mercenary services to those who can pay
relationships:
- target: '[[Cirhannim]]'
  type: custom
  metadata:
    description: exiled from
- target: '[[Hannanórë]]'
  type: custom
  metadata:
    description: banished from
- target: '[[Sun-Leaf Conclave]]'
  type: enemy
- target: '[[Valerius]]'
  type: custom
  metadata:
    description: potential mercenary contract
- target: '[[Sun-Leaf Conclave]]'
  type: custom
  metadata:
    description: exiled former members
timeline:
- period: ''
  event: Exiled from Hannanórë for extreme militancy
- period: ''
  event: Established a military encampment in the Crimson Wastes
tags:
- faction
- elven
- militant
- mercenary
created: '2026-03-15'
modified: '2026-03-15'
---
# Exiled Blades

The Exiled Blades are a band of [[Cirhannim]] warriors banished from [[Hannanórë]] for their extreme militancy. They believe that harmony can only be achieved by surgically excising all dissonance from the world with steel. Now based in the [[Crimson Wastes]], they operate as fanatical, disciplined mercenaries.

## Purpose & Goals

Unlike the contemplative [[Sun-Leaf Conclave]], the Exiled Blades believe the World-Song cannot be preserved through isolation and ritual alone. Dissonance must be cut out---literally. They see themselves as the blade that prunes corruption from the world, a necessary and violent instrument of harmony.

## Structure

The Exiled Blades operate as a **military meritocracy** led by a single **Warlord**, whose position is earned and held through duels of honor. Leadership is not inherited or appointed but won through martial prowess. Every member is a disciplined soldier, and rank is determined entirely by combat skill and tactical acumen.

## History

The Blades were once warriors of [[Hannanórë]], likely members of the Silent Sentinels caste or aligned with the militant elements of [[House Lyrandor]]. Their conviction that active military force---not passive concealment---was the answer to the world's growing dissonance brought them into direct conflict with the [[Sun-Leaf Conclave]]. The council declared their philosophy a *Song-Wrong* and exiled them. They crossed into the [[Crimson Wastes]], where the harsh environment forged them into an even harder fighting force.

## Notable Members

- The **Warlord** --- unnamed current leader, who holds command through undefeated duels of honor
