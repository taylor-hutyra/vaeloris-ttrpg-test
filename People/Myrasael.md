---
wb-type: person
wb-id: myrasael
name: Myrasael
aliases:
- Refinement
- The Elevating
- Farrazul
- Mirelaris
- The Jeweler
species: divine
born: null
died: null
birthplace: null
residence: null
faction: null
role: First Hand — embodiment of beauty, value, and perfection of form
title: First Hand of Refinement
gender: null
relationships:
- target: '[[Aúrastheon]]'
  type: created-by
  period: FA
  metadata:
    description: Brought forth in the Second Creation
- target: '[[Thamédran]]'
  type: sibling
  metadata:
    description: Fellow First Hand — together they created the Dwarves
- target: '[[Virandros]]'
  type: sibling
  metadata:
    description: Fellow First Hand — created together in the Second Creation
- target: '[[Elaraieth]]'
  type: sibling
  metadata:
    description: Fellow First Hand — created together in the Second Creation
- target: '[[Dwarves]]'
  type: creator
  period: FA
  metadata:
    description: Created with Thamédran — Dwarves embody methodical endurance and perfection-seeking
    nature: collaborative
- target: '[[Waldkyn]]'
  type: creator
  period: FA
  metadata:
    description: Created collaboratively with all four First Hands after the Paroxysm
    nature: collaborative
- target: '[[Illysia]]'
  type: creator
  period: FA
  metadata:
    description: Minor deity — the Craftmother, patron of artisans
- target: '[[Vaelynn]]'
  type: creator
  period: FA
  metadata:
    description: Minor deity — the Songkeeper, muse of music and poetry
- target: '[[Asteron]]'
  type: creator
  period: FA
  metadata:
    description: Minor deity — the Gemheart, master of precious materials
- target: '[[Kael''Zorai]]'
  type: enemy
  period: FA
  metadata:
    description: United with other First Hands to seal Kael'Zorai in the Binding
- target: '[[The Four Pillars]]'
  type: worshipped-by
  metadata:
    description: Known as Farrazul (Depth/Gem) in Dwarven tradition
- target: '[[The Four Harmonies]]'
  type: worshipped-by
  metadata:
    description: Known as Mirelaris (Gem shimmer) in Elven tradition
- target: '[[The Four Makers]]'
  type: worshipped-by
  metadata:
    description: Known as the Jeweler in Human tradition
- target: '[[The Synthesis]]'
  type: worshipped-by
  metadata:
    description: Known by true name Myrasael (Water/Refinement) in Waldkyn tradition
- target: '[[The Paroxysm]]'
  type: involved-in
  period: FA
  metadata:
    description: United with the other First Hands against Kael'Zorai in the Divine War
- target: '[[The Binding]]'
  type: involved-in
  period: FA
  metadata:
    description: Participated in stripping Kael'Zorai of name and authority
- target: '[[The Great Withdrawal]]'
  type: involved-in
  period: FA
  metadata:
    description: Withdrew into the realm of perfection
- target: '[[The Golden Age of Creation]]'
  type: involved-in
  period: FA
  metadata:
    description: Guided mortal races during the Golden Age alongside other First Hands
- target: '[[Artisan''s Codex]]'
  type: related
  period: FA
  metadata:
    description: Illysia, who created this codex, is an aspect of Myrasael
- target: '[[The Four Harmonies]]'
  type: worships
  metadata:
    description: First Hand of Refinement — known to Elves as Mirelaris
- target: '[[The Four Makers]]'
  type: worships
  metadata:
    description: First Hand of Refinement — known to Humans as the Jeweler
- target: '[[The Four Pillars]]'
  type: worships
  metadata:
    description: First Hand of Refinement — known to Dwarves as Farrazul
- target: '[[The Synthesis]]'
  type: worships
  metadata:
    description: First Hand of Refinement — the Waldkyn preserve the true name
- target: '[[The Twin Roots]]'
  type: related
  metadata:
    description: Illysia is an aspect of Myrasael (First Hand of Refinement)
- target: '[[Dwarves]]'
  type: created-by
  period: FA
  metadata:
    description: First Hand of Refinement — true divine parent alongside Thamédran
    nature: collaborative
- target: '[[Waldkyn]]'
  type: created-by
- target: '[[The Crystal Gardens]]'
  type: related
  period: FA
  metadata:
    description: Asteron, who created these gardens, is an aspect of Myrasael
- target: '[[The Divine Workshop]]'
  type: related
  period: FA
  metadata:
    description: Illysia, who founded this workshop, is an aspect of Myrasael
- target: '[[The Resonance Halls]]'
  type: related
  period: FA
  metadata:
    description: Vaelynn, who founded these halls, is an aspect of Myrasael
- target: '[[Fungril]]'
  type: creator
  period: FA
  metadata:
    description: Secondary Voice influence — Refinement
    nature: collaborative
- target: '[[Galapa]]'
  type: creator
  period: FA
  metadata:
    description: Secondary Voice influence — Refinement
    nature: collaborative
- target: '[[Katari]]'
  type: creator
  period: FA
  metadata:
    description: Primary Voice influence — Refinement
    nature: collaborative
timeline:
- period: FA
  event: Brought forth by Aúrastheon in the Second Creation
- period: FA
  event: 'First great work: inlaid the earth with veins of precious metals and gems'
- period: FA
  event: 'Created three minor deity aspects: Illysia, Vaelynn, and Asteron'
- period: FA
  event: Co-created the Dwarves with Thamédran — beings of methodical endurance and perfection
- period: FA
  event: United with the other First Hands against Kael'Zorai in the Paroxysm
- period: FA
  event: Co-created the Waldkyn as the final collaborative masterpiece of all four First Hands
- period: FA
  event: The Great Withdrawal — withdrew into the realm of perfection, presence felt in beauty, refinement, the pursuit of
    excellence
tags:
- person
- divine
- first-hand
- creator
- refinement
created: '2026-03-28'
modified: '2026-03-28'
---
# Myrasael

**Myrasael** is the First Hand of Refinement — one of the four divine beings brought forth by [[Aúrastheon]] in the Second Creation. Myrasael embodies beauty, value, and perfection of form. Of all the First Hands, Myrasael is that which elevates raw creation into meaningful expression.

## Domain and Essence

Myrasael's domain encompasses beauty in all its forms: precious metals and gems, artistry, the refinement of the raw into the sublime. Where [[Thamédran]] provides structure and [[Virandros]] brings life, Myrasael reveals the hidden beauty within both.

## First Great Work

Myrasael inlaid the earth of [[Vaeloris]] with veins of precious metals and gems — adorning the foundations [[Thamédran]] had laid with concentrated beauty.

## Mortal Creations

### The Dwarves

Through collaborative creation with [[Thamédran]], Myrasael brought forth the [[Dwarves]] — beings of methodical endurance who seek perfection in their craft. The Dwarves embody the combined essences of Stability and Refinement: masterful craftspeople who create objects of enduring beauty.

### The Waldkyn

After the Paroxysm, Myrasael joined with the other three First Hands to create the [[Waldkyn]].

## Minor Deity Aspects

Myrasael created three aspects to serve as focused guides:

- **[[Illysia]]** the Craftmother — patron of artisans and refined creation
- **[[Vaelynn]]** the Songkeeper — muse of music, poetry, and harmonic expression
- **[[Asteron]]** the Gemheart — master of precious materials and concentrated beauty

## Cultural Names

- **Farrazul** (Depth/Gem) — the Dwarven Pillar of hidden beauty refined through labor
- **Mirelaris** (Gem shimmer) — the Elven Harmony of hidden beauty and inner worth
- **The Jeweler** — the Human Maker who adorned the depths with beauty
- **Myrasael** (Water/Refinement) — the Waldkyn Voice, true name preserved

## The Great Withdrawal

After the creation of the Waldkyn, Myrasael withdrew into the realm of perfection. Myrasael's presence is felt in beauty, refinement, and the pursuit of excellence. The legacy: the drive to create meaning and value.
